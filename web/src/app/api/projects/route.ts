...
export async function POST(request: Request) {
	...
	try {
		...
		const project = {
			...
			description,
			category,
			asset_focus: asset_focus || null,
			asset_type: asset_type || null,
			issuance_model: issuance_model || null,
			compliance_model: compliance_model || null,
			audit_status: audit_status || null,
			issuer_account_id: issuer_account_id || null,
			stellar_toml_url: stellar_toml_url || null,
			supported_markets: supported_markets || null,
			trading_pairs: trading_pairs || null,
			on_chain_volume: on_chain_volume || null,
			verification_sources: verification_sources || null,
			status: "submitted",
