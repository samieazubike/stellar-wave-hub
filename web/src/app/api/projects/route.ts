...
export async function GET(request: Request) {
    try {
        ...
        // Firestore search handled client-side
        if (search) {
        ...
        ratingsSnap.docs.forEach((d) => {
            ...
            if (!ratingsByProject.has(pid)) {
                ratingsByProject.set(pid, []);
            }
        ...
        enriched.sort((a, b) => {
            ...
            if (
                (b.featured as number) !==
                (a.featured as number)
            ) {
                return (
                    (b.featured as number) -
                    (a.featured as number)
...
export async function POST(request: Request) {
    ...
    const { success, resetTime } = rateLimit(`projects_post_${ip}`, 10, 60 * 1000);
    if (!success) {
        return Response.json(
            { error: "You are submitting projects too quickly. Please take a break." },
